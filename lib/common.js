...
async function mergeStyles(sourceFolderPath, outputFilePath, options) {
  try {
    let elements = await readdir(sourceFolderPath, { withFileTypes: true });
    if (options) {
      elements = elements.sort((a, b) => {
        return options.sortingArr.indexOf(a.name) >=
          options.sortingArr.indexOf(b.name) ?
          1 :
          -1;
      })
    }
